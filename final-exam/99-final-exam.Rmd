...
When the two variables are independent, the conditional distribution is the same as the marginal distribution. The prediction errors are the same in this situation.

When the two variables are dependent, we always use the mean square error to evaluate the prediction error. Considering that the MSE is equal to variance plus squared bias and a good estimator is always an unbiased estimator, we just need to compare the variance of two situations. As the ocnditional variance is less than the variance, we can get that the prediction from conditional distribution have smaller prediction error than predictions from marginal distribution.
